...
- Wave 1: two `Enemy_01` v8 Mid meshes + 5 large asteroids. Later: Scout v7 / Gunner v7 / Drone v6. Waves 7+: Bomber v8 / Sniper v8 / SwarmPod v6 if those FBX imported. Wave 4+ adds **Brute** (`Monster_Brute` charge tank). Wave 6+ adds **Swarm** (`Monster_Swarm` spawner → Swarmling minions). Sniper spawn-fallback matches Scout/Gunner. After wave 10: +1 large asteroid per wave (cap 10). Fail text `Enemy contact (Kind)` including Brute / Swarm. Arena hazard contact reads **Arena hazard**.
