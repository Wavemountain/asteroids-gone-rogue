...
This is the Week 1–2 playable core for **Asteroids gone rogue**. Branch: `0.34-enemies-launch` (do not merge into older version tags; no 0.35).
...
**ArtImport: 51/51 Play Mode FBX ready** — `ArtImport.PlayModeAssets` has 51 names; all 51 exist under `Assets/Resources/Art/Import/` and `Assets/Art/Import/`. `Ship_Body_Upgrade02` is imported only (not warmed, not in the shop). 0.34 art-wire adds Bomber / Sniper v5 (canonical `Enemy_Bomber` / `Enemy_Sniper`) and hangar `Hangar_LaunchSign`. Buffer_* names are aliases only; primary FBX names resolve first.

- Hangar shows BlenderBot ship + crate / terminal / pillar / workbench / kiosk / banner / ammo rack + **Console / PowerBox / FireExtinguisher / Locker / LaunchSign** + parked **Ship_Complete**. LaunchSign sits by the pad as the Start Wave landmark.
...
- Wave clear / fail show a short **Run summary** card: score / wave / world / credits (+ awarded) / upgrades. New record appends **NEW BEST**. After waves 1–3, one short continue line (World 2 / Gunner unlock + next shop buy).
