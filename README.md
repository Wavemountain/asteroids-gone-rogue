...
- **Arena World 2–7** meshes **do spawn** — after every 5 cleared waves the floor swaps `Arena_AstroFloor` (World 1 / 7, aliased over `Arena_Blockout`) → World2 → … → World6, then World 7 **Spoke ring** reuses the World 1 AstroFloor with a new layout (pylons / trench / mine belt / cross / islands / spokes). `ArenaEnv` keeps additive parallax stars (far tint 0.45 / near 1.15), dual nebula domes, a 22-rock silhouette belt, a 0.7× cyan grid-fade, a cooler floor, and a play-only vignette. Debris islands prefer `Arena_RockIsland_A`. `Arena_Hazard_Spike` FBX marks hazards. Same wrap / roster rules; floor scales with arena radius 30. FOV stays 54.
