...
| Start / next / retry wave | Hangar **Start Wave** / **Next Wave**. 0 lives returns to hangar / start (full run reset), not Retry Wave |
...
| Buy upgrade | Hangar shop buttons. Ship sits on the **right** in a framed RenderTexture viewport (`ShipPreviewFrame` 0.562–0.986, `ShowcaseScale` 2.25), idle-spins, shows bought parts; hover / pad-focus previews the upgrade |

## Week 1 loop

`GameSession` / `GameManager` / `WaveManager` states:

1. **Hangar** — title, start button, shop (if you have credits). First session shows a dismissable **First flight** card (WASD / Abort Esc / Q·RMB fire modes / shop / Start Wave / medal ladder → ★ Scout Wing at wave 3; PlayerPrefs). A **LANG** plate sits left of the audio mixers: click the smaller, quieter **US flag** for English or the **Swedish flag** for Swedish (PlayerPrefs `agr.ui.language`, `en` / `sv`). HUD, shop, hints, credits, fail UI, hangar status, and layout flash all read `Loc` keys. Hangar dressing includes Console, PowerBox, FireExtinguisher, Locker, and a **LaunchSign** landmark (emissive GO plate + mesh GO decal) on the pad’s camera-front edge. Shop sits on the **left** (`HangarPanel` 0.014–0.55); the playable ship is on the **right** in a framed RenderTexture `ShipPreviewFrame` (0.562–0.986 × 0.080–0.708, hero / `ShowcaseScale` 2.25, idle spin, bought parts + hover ghost). A dedicated overlay canvas (`ShipPreviewCanvas`, sort 80) keeps the amber box visible. The studio parks behind the hangar camera (`StudioZ` −140, layer 8) and Default-layer ship renderers are disabled so the world ship cannot leak through Weapons/Defense. The **medal ladder** is labeled **MEDALS** (Scout Wing / Deep Orbit / Far Drift) in hangar **and** play (`★` earned / `○` locked; `agr.hangar.medals`, capacity 3). Hangar status shows the next-medal hook.
...
4. **Fail** — ship destroyed; retry the same wave. Bought upgrades stay. Fail names the cause as **your hull** (`SHIP LOST · {reason}` + “that was you”), adds an **Almost had it** line when 1–3 threats remain, and uses a steel/amber fail card (Kenney Future, anti-epa). The **HEALTH** rack stays visible. Hangar HUD keeps **Best** visible.
...
- **Arena World 2–7** meshes **do spawn** — after every 5 cleared waves the floor swaps `Arena_AstroFloor_v2` (World 1 / 7, Play Mode alias of `Arena_Blockout`; 7 plates + `Mat_AstroRim`) → World2 → … → World6, then World 7 **Spoke ring** reuses that v2 floor with a new layout (pylons / trench / mine belt / cross / islands / spokes). `ArenaEnv` keeps additive parallax (Far tile 3.6 / Near 1.05), dual nebula (opacity 0.34, chroma ×0.55), a 22-rock belt (Y −0.5…3.2), a 0.045 grid, cool-white underside glow, and no DustRing disk so stars leak between chunks. Debris islands prefer `Arena_RockIsland_A`. `Arena_Hazard_Spike` FBX marks hazards. Same wrap / roster rules; `ArenaBounds` stays radius 30. FOV stays 54.
...
- Combat juice: light screen flash + camera shake on hits / explosions. Player death stays quiet on flash/shake; fail plays `GameOver` + `lowDown` (`phaserDown3` fallback).
