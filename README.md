...
- **Arena World 2–7** meshes **do spawn** — after every 5 cleared waves the floor swaps `Arena_AstroFloor` (World 1 / 7, aliased over `Arena_Blockout`) → World2 → … → World6, then World 7 **Spoke ring** reuses the World 1 AstroFloor with a new layout (pylons / trench / mine belt / cross / islands / spokes). `ArenaEnv` keeps a parallax starfield, nebula dome, asteroid-belt ring, and cyan grid-fade across swaps. Debris islands prefer `Arena_RockIsland_A`. `Arena_Hazard_Spike` FBX marks hazards. Same wrap / roster rules; floor scales with arena radius 30.
