...
- **Arena World 2–7** meshes **do spawn** — after every 5 cleared waves the floor swaps `Arena_AstroFloor` (World 1 / 7, aliased over `Arena_Blockout`; prefers `AstroFloor_v2` when that FBX lands) → World2 → … → World6, then World 7 **Spoke ring** reuses the World 1 AstroFloor with a new layout (pylons / trench / mine belt / cross / islands / spokes). `ArenaEnv` keeps additive parallax (Far tile 3.6 / Near punch 1.38), dual nebula, a 22-rock belt (Y −0.5…3.2), a 0.06 grid, underside PointLight glow, and no DustRing disk so stars leak between chunks. Debris islands prefer `Arena_RockIsland_A`. `Arena_Hazard_Spike` FBX marks hazards. Same wrap / roster rules; `ArenaBounds` stays radius 30. FOV stays 54.
